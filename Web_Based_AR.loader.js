function createUnityInstance(canvas, config, onProgress) {
    ...
    // Safari uses webkitfullscreenchange event and not fullscreenchangedocument.addEventListener("webkitfullscreenchange",webkitFullscreenChangeEventHandler);
    ...
    Module.SystemInfo = (function () {
        ...
        var oses = [
            ...
            ['FreeBSD( )', 'FreeBSD'],
            ['OpenBSD( )', 'OpenBSD'],
            ['Linux|X11()', 'Linux'], ['Mac OS X ([0-9_\\.]+)', 'MacOS'],
    ...
    /*gl.RENDERER*/);
        ...
        return {
            ...
            hasWebGL: glVersion, hasCursorLock: !!document.body.requestPointerLock,
    ...
    Module.readBodyWithProgress = function () {
        ...
        * @param {Response} response A Fetch API response object* @param {boolean} lengthComputable Return wether content length was given in
        header.
        * @returns {number}
        */
    ...
    // More data received than estimated, create new larger body to prepend all additional chunks to the body
            ...
            return newBody; return readBody().then(function (parsedBody) {
                onProgress({
                    type: "load",
                    response: response,
    ...
    */Module.UnityCache = function () {
    ...
    // On very first page load opening a connection to IndexedDB hangs without triggering onerror.
                    ...
                    openRequest.onupgradeneeded =
                        self.upgradeDatabase.bind(self); openRequest.onsuccess = function (e) {
                            clearOpenDBTimeout();
    ...
    indexedDB cache due to the error: " + error);
        ...
        return unityCache.loadRequest(url).then(function (result) {
            ...
            if (cache.control == "immutable") {
                cache.revalidated = true; unityCache.updateRequestMetaData(metaData).then(function () {
...
function downloadBinary(urlId) {
    ...
    var cacheControl = Module.cacheControl(Module[urlId]); var fetchImpl = Module.companyName && Module.productName ? Module.cachedFetch :
...
function loadBuild() {
    ...
    var dataPromise = downloadBinary("dataUrl"); Module.preRun.push(function () {
